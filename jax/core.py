...
def sorted_named_shape_tuples(named_shape_tuples):
  # Sort axis name strings alphabetically, and choose an arbitrary but fixed
  # order for other hashable types.
  return sorted(set(named_shape_tuples),
                key=lambda x: x[0] if isinstance(x[0], str) else hash(x[0]))
...
def typematch(aval1: AbstractValue, aval2: AbstractValue) -> bool:
  ...
  if hasattr(aval1, 'named_shape') and hasattr(aval2, 'named_shape'):
